<form action="/tugasmaster/{{ $tugasmaster->id }}/update" method="POST" id="frmtugasmaster"
    ...
    <div class="row">
        <div class="col-12">
            <select name="matkul_id" id="matkul_id" class="form-select">
                ...
                @foreach ($matkul as $d)
                    <option {{ $tugasmaster->matkul_id == $d->id ? 'selected' : '' }} value="{{ $d->id }}">
                        {{ $d->nama_matkul }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-12">
            <select name="pertemuan_id" id="pertemuan_id" class="form-select">
                ...
                @foreach ($pertemuan as $d)
                    <option {{ $tugasmaster->pertemuan_id == $d->id ? 'selected' : '' }}
                        value="{{ $d->id }}">
                        {{ $d->nama_pertemuan }}
                    </option>
                @endforeach
        ...
        <div class="col-12">
            <select name="mahasiswa_id" id="mahasiswa_id" class="form-select">
                ...
                @foreach ($mahasiswa as $d)
                    <option {{ $tugasmaster->mahasiswa_id == $d->id ? 'selected' : '' }}
                        value="{{ $d->id }}">
                        {{ $d->name }}
                    </option>
                @endforeach
